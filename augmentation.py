...
class InverteThreshold:
    def __init__(self, mean_thresh):
        self.mean_thresh = mean_thresh

    def __call__(self, img):
        if img.mean() > self.mean_thresh:
...
def augment_train(x, mask=None):
    return DualCompose([
        ImageOnly(InverteThreshold(130.0)),
        ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.10, rotate_limit=45, prob=0.25),
        RandomFlip(prob=0.5),
        Transpose(prob=0.5),
        ImageOnly(RandomContrast(limit=0.1, prob=0.5)),
